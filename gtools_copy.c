...
initialGraph=createVectorList(n,3);
for(i=0;i<n;i++)
fscanf(input,"%d %d %d",&initialGraph[i][0],&initialGraph[i][1],&initialGraph[i][2]);
fclose(input);
...
fscanf(input,"%d",&m);
mutList=createVectorList(m,3);
for(i=0;i<m;i++)
fscanf(input,"%d %d %d",&mutList[i][0],&mutList[i][1],&mutList[i][2]);
fclose(input);

sprintf(fileName,"graph_g%d.edgelist_copy",g);
...
for(i=0;i<n;i++)
fprintf(input,"%d %d %d\n",initialGraph[i][0],initialGraph[i][1],initialGraph[i][2]);

for(i=0;i<g;i++)
fprintf(input,"%d %d %d\n",mutList[mut[i]][0],mutList[mut[i]][1],mutList[i][2]);
